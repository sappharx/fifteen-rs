...
impl Board {
    ...
    pub fn move_tile(&self, tile: u8) -> Result<(), Error> {
        if tile == 0 {
        ...
        let is_valid_move = match (self.get_coordinates(tile), self.get_coordinates(0)) {
            (None, _) => { return Err( Error::new("tile doesn't exist on board".to_string()) ); },
            (_, None) => { panic!("can't find empty space on board"); },
            (Some(tile_coords), Some(zero_coords)) => {
